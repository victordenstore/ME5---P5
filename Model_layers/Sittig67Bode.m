...
f = linspace(10000,2*10^6,1000);                                                               % operating frequency [Hz]
...
r_copper = 2.5*10^-3;
S_copper = pi*r_copper^2;
rho_copper = 8920;
c_copper = 3570;
d_copper = 0.5*10^-3;
v_copper = sqrt(c_copper/rho_copper);

Z_copper = S_copper*rho_copper*c_copper;
f_0_cu = v_copper/(2*d_copper);
gamma_copper = pi*f/f_0_cu;
C_copper = eps_0*S_copper/d_copper;


A_copper = [cos(gamma_copper) j*Z_copper*sin(gamma_copper); ...
    (j*sin(gamma_copper))/Z_copper cos(gamma_copper)];
A_E_copper = 1;
B_E_copper = 1./(j*omega*C_copper);
C_E_copper = 0;
D_E_copper = 1;

T_intermediate_copper = zeros(4*length(f),4);
...
for ee = 1:length(f)
    ...
    row4 = 4*ee;
    rows = [row1 row2 row3 row4];
A_copper(rows(1):rows(2),1:2) = [cos(gamma_copper(ee)) j*Z_copper*sin(gamma_copper(ee)); ...
    (j*sin(gamma_copper(ee)))/Z_copper cos(gamma_copper(ee))];
A_copper_now = A_copper(rows(1):rows(2),1:2);
T_intermediate_copper(rows(1):rows(4),1:4) = [A_copper_now(1,1) A_copper_now(1,2) 0 0; ...
                    A_copper_now(2,1) A_copper_now(2,2) 0 0; ...
                    0 0 A_E_copper B_E_copper(ee); ...
                    0 0 C_E_copper D_E_copper];
end
...
for tt = 1:length(f)
    ...
    row4 = 4*tt;
    rows = [row1 row2 row3 row4];
T(rows(1):rows(4),1:4) = Tn(rows(1):rows(4),1:4)*Tn(rows(1):rows(4),1:4)* (T_intermediate_copper(rows(1):rows(4),1:4))^2 * T_intermediate_FP(rows(1):rows(4),1:4);
...
f_o_H = v_H/(2*d_H)
f_o_H = v_H/(2*d_H);
gamma_H = pi*f/f_o_H;Z_H = rho_H*c_H*S_H;
...
C_b = A_B(entry2,1);
D_b = A_B(entry2,2);
Z_b(kl) = (A_b*Z_B + B_b)/(C_b*Z_B + D_b);
end
...
A_c(jj) = T_31(jj) - T_33(jj).*(T_21(jj).*Z_b(jj) + T_11(jj))./(T_23(jj).*Z_b(jj) + T_13(jj));
B_c(jj) = T_32(jj) - T_33(jj).*(T_22(jj).*Z_b(jj) + T_12(jj))./(T_23(jj).*Z_b(jj) + T_13(jj));
...
figure
subplot(2,1,1); plot(f*10^-6,mag_impedance); title('Frequency response of impedance'); ...
    xlabel('Frequency [MHz]'); ylabel('Magnitude [\Omega]'); legend('theoretical impedance response'); grid on;
subplot(2,1,2); plot(f*10^-6,phase_impedance); title('Phase plot of the impedance'); legend('theoretical impedance phase'); grid on; ...
...
figure
subplot(2,1,1);  plot(f*10^-6,mag_tf); title('Frequency response of S_{FV}'); ...
    xlabel('Frequency [MHz]'); ylabel('magnitude [N/V]');  legend('theoretical system frequency response'); grid on;
subplot(2,1,2); plot(f*10^-6,phase_tf); ...
    xlabel('Frequency [MHz]'); ylabel('Phase [degree]');  legend('theoretical system phase'); grid on;
