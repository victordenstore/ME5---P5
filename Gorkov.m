...
function [v,p_mean_square,v_in,v_mean_square,Uac,Fac] = Gorkov(P_surface,w,lambda,R1,R2,T,rho_l,r,f_1,f_2,c,B,damp_coeff,L)
...
P_field = P_surface*(R1+1) * exp(1i*w*t-1i*x*2*pi/lambda)*exp(- damp_coeff * x) ...     % The standing pressure field wave expression
         - R2*P_surface * exp(1i*w*t+1i*(x)*2*pi/lambda)*exp(- damp_coeff * (L-x));
